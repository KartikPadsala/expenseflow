...
export interface SettlementUser { id: string; displayName: string; avatarUrl?: string; email?: string; }
export interface SettlementGroup { id: string; name: string; currency: string; }
...
export function useCreateSettlement() {
  ...
  return useMutation({
    mutationFn: async (body: { payeeId: string; amount: number; currency: string; method?: string; groupId?: string; notes?: string }) => {
      const { data } = await api.post('/settlements', body);
